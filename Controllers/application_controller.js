...
const getAppliedjobs = async (req, res) => {
    try {
        ...
        const application = await Application.find({ applicant: userid }).populate({ path: "job" , populate:{path : "created_by"} }).populate({ path: "applicant" })
        if(!application){
...
try {
    ...
    if(!status){
        return res.status(200).josn({ message:"status required", success:false})
...
return res.status(200).json({ message:"status update successfully", success :true})
